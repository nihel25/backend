...
public class UserImageStorageImpl implements ImageStorage {
    ...
    @Autowired
    public UserImageStorageImpl(tn.esprit.formation.configimage.FileStorageProperties fileStorageProperties) {
        this.imageLocation = Paths.get(fileStorageProperties.getUploadImgUsersDir()).toAbsolutePath().normalize();
        try {

        } catch (Exception e) {
            throw new tn.esprit.formation.exceptions.FileStorageException("could not create the directory where the uploaded images will be stored", e);
        }
    }
    ...
    public String store(MultipartFile file) {
        ...
        try {
            if (fileName.contains("..")) {
                throw new tn.esprit.formation.exceptions.FileStorageException("File name contains invalid path sequence " + fileName);
            }
